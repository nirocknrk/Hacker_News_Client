package hn.nrk.com.hackernewsclient.views.recyclerview.adapter;
...
public class RecyclerViewAdapter extends RecyclerView.Adapter<RecyclerViewAdapter.ViewHolder> {
    ...
    private List<HNStory> items;

    public RecyclerViewAdapter(Cursor cursor, NewsListener listener) {
        items = new ArrayList<>();
        if (cursor != null) {
            for (int i = 0; cursor.moveToNext(); i++) {
                add(HNStory.fromBookmark(cursor), i);
            }
        }
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        final HNStory story = items.get(position);
    ...
    private void add(HNStory item, int position) {
